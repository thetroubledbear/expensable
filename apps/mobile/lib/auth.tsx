...
import { authSignIn, authGetSession } from "./api"
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  async function init() {
    ...
    if (session?.user) {
      ...
      // No active session — try re-auth from saved credentials
  ...
  async function register(name: string, email: string, password: string): Promise<string | null> {
    try {
      const res = await fetch("https://expensable-gamma.vercel.app/api/auth/register", {
        method: "POST",
